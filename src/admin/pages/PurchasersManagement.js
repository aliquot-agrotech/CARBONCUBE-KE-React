...
const PurchasersManagement = () => {
  ...
  useEffect(() => {
      const fetchPurchasers = async () => {
          try {
              const response = await fetch(`http://127.0.0.1:3001/admin/purchasers?search_query=${searchQuery}`, {
                  headers: {
  ...
  const handleRowClick = async (purchaserId) => {
    try {
      const response = await fetch(`http://127.0.0.1:3001/admin/purchasers/${purchaserId}`, {
        headers: {
  ...
  const handleUpdateStatus = async (purchaserId, status) => {
    try {
      const response = await fetch(`http://127.0.0.1:3001/admin/purchasers/${purchaserId}/${status}`, {
        method: 'PUT',
        headers: {
