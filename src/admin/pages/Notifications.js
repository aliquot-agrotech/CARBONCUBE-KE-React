...
const Notifications = () => {
    ...
    useEffect(() => {
        const fetchAdminId = async () => {
            ...
            if (token) {
                try {
                    const response = await fetch('https://carboncube-ke-rails-cu22.onrender.com/admin/identify', {
                        headers: {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            const token = sessionStorage.getItem('token');
            const response = await fetch('https://carboncube-ke-rails-cu22.onrender.com/admin/notifications', {
                headers: {
                    'Authorization': 'Bearer ' + token,
                },
            });
