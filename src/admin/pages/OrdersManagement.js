...
const OrdersManagement = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:3001/admin/orders?search_query=${searchQuery}`, {
                    headers: {
    ...
    const handleRowClick = async (orderId) => {
        try {
            const response = await fetch(`http://127.0.0.1:3001/admin/orders/${orderId}`, {
                headers: {
                    'Authorization': 'Bearer ' + sessionStorage.getItem('token'),
                },
            });
    ...
    const handleUpdateStatus = async (orderId, status) => {
        try {
            const response = await fetch(`http://127.0.0.1:3001/admin/orders/${orderId}/update-status`, {
                method: 'PUT',
                headers: {
