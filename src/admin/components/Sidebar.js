...
import { House, Box, Truck, Person, FileText, Percent, Envelope, Bell, GraphUp, List, X } from 'react-bootstrap-icons';
...
const Sidebar = () => {
  ...
  return (
    <>
      <Button variant="warning" className={`toggle-button ${isOpen ? 'd-none' : 'd-block'}`} onClick={toggleSidebar}>
        <List />
      </Button>
      <div className={`sidebar ${isOpen ? 'open' : 'collapsed'}`}>
        <Button variant="warning" className="toggle-button d-md-none" onClick={toggleSidebar}>
          <X />
        </Button>
