...
const Home = () => {
    ...
    useEffect(() => {
        const fetchCategoriesAndAds = async () => {
            try {
                const categoryResponse = await fetch('http://carboncube-backend:3001/purchaser/categories', {
                    headers: {
                ...
                const adResponse = await fetch('http://carboncube-backend:3001/purchaser/ads', {
                    headers: {
                        'Authorization': 'Bearer ' + sessionStorage.getItem('token'),
                    },
                });
    ...
    const handleSearch = async (e, category = 'All', subcategory = 'All') => {
        ...
        try {
            ...
            const response = await fetch(
                `http://carboncube-backend:3001/purchaser/ads/search?query=${encodeURIComponent(searchQuery)}&category=${category}&subcategory=${subcategory}`, 
                {
                    headers: {
                        'Authorization': 'Bearer ' + sessionStorage.getItem('token'),
                    },
    ...
    const logAdSearch = async (query, category, subcategory) => {
        try {
            const logResponse = await fetch('http://carboncube-backend:3001/ad_searches', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + sessionStorage.getItem('token'),
