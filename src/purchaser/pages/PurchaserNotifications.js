...
const PurchaserNotifications = () => {
    ...
    useEffect(() => {
        const fetchPurchaserId = async () => {
            ...
            if (token) {
                try {
                    const response = await fetch('https://carboncube-ke-rails-cu22.onrender.com/purchaser/identify', {
                        headers: {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            const token = sessionStorage.getItem('token');
            const response = await fetch('https://carboncube-ke-rails-cu22.onrender.com/purchaser/notifications', {
                headers: {
                    'Authorization': 'Bearer ' + token,
                },
            });
