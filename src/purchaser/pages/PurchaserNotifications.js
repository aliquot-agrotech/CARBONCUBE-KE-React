...
const PurchaserNotifications = () => {
    ...
    useEffect(() => {
        const fetchPurchaserId = async () => {
            ...
            if (token) {
                try {
                    const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/purchaser/identify`, {
                        headers: {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            const token = sessionStorage.getItem('token');
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/purchaser/notifications`, {
                headers: {
                    'Authorization': 'Bearer ' + token,
                },
            });
