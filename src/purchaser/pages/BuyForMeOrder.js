...
const Orders = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await fetch(`http://127.0.0.1:3001/purchaser/orders?purchaser_id=${purchaserId}&search_query=${searchQuery}`, {
                    headers: {
    ...
    const handleRowClick = async (orderId) => {
        try {
            const response = await fetch(`http://127.0.0.1:3001/purchaser/orders/${orderId}`, {
                headers: {
                    'Authorization': 'Bearer ' + sessionStorage.getItem('token'),
                },
            });
