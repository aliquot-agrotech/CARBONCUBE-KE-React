...
const PurchaserMessages = () => {
  ...
  if (token) {
    ...
    const userType = payload.role || payload.type || 'purchaser';
  ...
  const fetchMessages = async (conversationId) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/purchaser/conversations/${conversationId}/messages`, {
        headers: {
          'Authorization': 'Bearer ' + sessionStorage.getItem('token'),
        },
      });
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/purchaser/conversations/${selectedConversation.id}/messages`, {
        method: 'POST',
        headers: {
