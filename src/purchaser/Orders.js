...
const Orders = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const response = await fetch(`https://carboncube-ke-rails-qrvq.onrender.com/purchaser/orders?purchaser_id=${purchaserId}&search_query=${searchQuery}`, {
                    headers: {
    ...
    const handleRowClick = async (orderId) => {
        try {
            const response = await fetch(`https://carboncube-ke-rails-qrvq.onrender.com/purchaser/orders/${orderId}`, {
                headers: {
                    'Authorization': 'Bearer ' + sessionStorage.getItem('token'),
                },
            });
