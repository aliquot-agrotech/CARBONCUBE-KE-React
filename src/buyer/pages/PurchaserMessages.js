...
const PurchaserMessages = () => {
  ...
  if (token) {
    ...
    const userType = payload.role || payload.type || 'buyer';
  ...
  const fetchMessages = async (conversationId) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/buyer/conversations/${conversationId}/messages`, {
        headers: {
          'Authorization': 'Bearer ' + sessionStorage.getItem('token'),
        },
      });
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/buyer/conversations/${selectedConversation.id}/messages`, {
        method: 'POST',
        headers: {
                      ...
                      <Card className="select-conversation-card text-center">
                        <Card.Body>
                          <div className="select-conversation-text">
                            💬 Start chatting with your sellers or buyers
                          </div>
                        </Card.Body>
                      </Card>
                    </div>
