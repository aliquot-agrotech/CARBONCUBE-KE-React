...
// import '../css/PurchaserNotifications.css';  // Custom CSS file
...
const PurchaserNotifications = () => {
    ...
    const [buyerId, setPurchaserId] = useState(null);
    ...
    useEffect(() => {
        const fetchPurchaserId = async () => {
            ...
            if (token) {
                try {
                    const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/buyer/identify`, {
                        headers: {
                    ...
                    setPurchaserId(data.buyer_id);
                } catch (error) {
                    // console.error('Error fetching buyer ID:', error);
                    setError('Error fetching buyer ID');
                } finally {
                    setLoading(false);
                }
            } else {
    ...
    useEffect(() => {
        if (buyerId) {
            ...
            const subscription = consumer.subscriptions.create(
                { channel: 'NotificationsChannel', buyer_id: buyerId },
                {
                    received: (data) => {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            const token = sessionStorage.getItem('token');
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/buyer/notifications`, {
                headers: {
                    'Authorization': 'Bearer ' + token,
                },
            });
