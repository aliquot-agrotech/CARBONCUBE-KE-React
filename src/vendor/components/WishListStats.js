            ...
            <div className="card shadow-sm mt-2" style={{ backgroundColor: 'rgba(255, 255, 255, 0.1)', border: 'none' }}>
                <div className="card-body px-2">
                    ...
                    {wishlist_conversion_rate.map((product, index) => (
                        <li key={index} className="list-group-item d-flex justify-content-between align-items-center" style={{ backgroundColor: 'rgba(255, 255, 255, 0.1)', border: 'none' }}>
                        {product.ad_title}
                        <span className={`badge ${product.wishlist_count === 0 ? 'bg-secondary' : 'bg-success'}`}>
