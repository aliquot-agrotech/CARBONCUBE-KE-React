...
import './VendorNotifications.css';  // Custom CSS file
...
const VendorNotifications = () => {
    ...
    useEffect(() => {
        const fetchVendorId = async () => {
            ...
            if (token) {
                try {
                    const response = await fetch('https://carboncube-ke-rails-qrvq.onrender.com/vendor/identify', {
                        headers: {
                    ...
                    setError('Error fetching vendor ID');
                } finally {
                    setLoading(false);
                }
            } else {
    ...
    useEffect(() => {
        const fetchNotifications = async () => {
            const token = sessionStorage.getItem('token');
            const response = await fetch('https://carboncube-ke-rails-qrvq.onrender.com/vendor/notifications', {
                headers: {
                    'Authorization': 'Bearer ' + token,
                },
            });
